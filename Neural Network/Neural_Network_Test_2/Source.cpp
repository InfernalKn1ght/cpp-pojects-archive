...
double input_layer[784], hidden_layer[200], output_layer[10], weight0[784][784], weight1[784][784], bias0[200], bias1[10], ideal_mass[10], delta_o[784], mass_del_w0[784][784], mass_del_w1[784][784], mass_del_bw0[200], mass_del_bw1[10], input_layer1[784];
...
int main()
{
	...
	fill_matrix(weight0, 200, 784);
	fill_matrix(weight1, 10, 200);

	zero_mass(mass_del_w0, 200, 784);
	zero_mass(mass_del_w1, 10, 200);

	zero_mass1(mass_del_bw0, 200);
	...
	while (iteration < 60000)
	{
		...
		activation(weight0, input_layer, hidden_layer, bias0, 200, 784);
		activation(weight1, hidden_layer, output_layer, bias1, 10, 200);

		cost_f(iteration);

		delta_out_l();

		delta_hid_l(weight1, delta_o, hidden_layer, 200, 10);
		delta_bias_w(delta_o, mass_del_bw1, bias1, 10); //bias1 update
		delta_weights(delta_o, weight1, hidden_layer, 200, 10, mass_del_w1); //weight1 update

		delta_bias_w(delta_h, mass_del_bw0, bias0, 200); //bias0 update
		delta_weights(delta_h, weight0, input_layer, 784, 200, mass_del_w0); //weight0 update
